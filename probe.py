...
def blacklist_kernel_modules(module_list):
    ...
    logger.info(f"Create blacklist file for unwanted kernel modules: [{*module_list,}]")
        ...
        for vendor_id, model_id in MODEM_IDS:
            modem_device = list(
                context.list_devices(
                    subsystem="usb", ID_VENDOR_ID=vendor_id, ID_MODEL_ID=model_id
                )
            )  # alternatively a more generic filter could be used?
            if modem_device:
                break
        time.sleep(1)
...
def main():
    ...
    if parser.is_measurement_namespace_enabled():
        ...
        if parser.is_debug_bridge_enabled():
            ...
            ps_portforward = pexpect.spawn(
                f"socat tcp-listen:{ProbeParser.DEFAULT_NETNS_DEBUGGING_PORT},reuseaddr,fork tcp-connect:10.29.183.1:{ProbeParser.DEFAULT_NETNS_DEBUGGING_PORT}"
            )

    # start measurements with namespace wrapper script
    ps_test = pexpect.spawn(
        f"./mobileatlas/probe/enter_netns.py {parser.get_test_name()} {parser.get_args()}",
    ...
    with path.open("a") as f:
        f.write(
            f"0,{parser.get_test_name()},{start.isoformat(timespec='seconds')},{stop.isoformat(timespec='seconds')}"
        )
